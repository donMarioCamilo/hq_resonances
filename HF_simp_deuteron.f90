...
!In this demo the heavy deuteron ccb-bbc ground state will be computed
PROGRAM HF_deuteron
    IMPLICIT none
    ...
    DOUBLE PRECISION, PARAMETER         ::  alpha_cb = sqrt(alpha_charm*alpha_bottom)
    !Bohr radius
    DOUBLE PRECISION, PARAMETER         ::  bohr_r = 1.d0/(sqrt(mass_charm*mass_bottom)*alpha_cb)

    !radial coordinates
    DOUBLE PRECISION, ALLOCATABLE       ::  r(:)
    DOUBLE PRECISION, PARAMETER         ::  r0 = 0.001d0*bohr_r, rmax = 1.5d1*bohr_r
    
    !Initial guess function
    DOUBLE PRECISION, ALLOCATABLE       ::  phi_init_charm1(:),phi_init_bottom1(:)
    DOUBLE PRECISION, ALLOCATABLE       ::  phi_init_charm2(:),phi_init_bottom2(:)

    !Non-iterable matrix
    DOUBLE PRECISION, ALLOCATABLE       ::  h_charm(:,:),h_bottom(:,:)
    !Iterables
    !Eigenvalue resulting function
    DOUBLE PRECISION, ALLOCATABLE       ::  phi_charm1(:),phi_bottom1(:), F_matrix_charm1(:,:),F_matrix_bottom1(:,:)
    DOUBLE PRECISION, ALLOCATABLE       ::  phi_charm2(:),phi_bottom2(:), F_matrix_charm2(:,:),F_matrix_bottom2(:,:)
    DOUBLE PRECISION                    ::  energy_new, energy_old, eps_charm1, eps_bottom1,eps_charm2,eps_bottom2
    !Parameters
    !Orbital spacing    
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !Initial guess for the orbital
    phi_init_charm1 = (2.d0*sqrt((mass_charm*alpha_charm)**3)) * r * EXP(-r*(mass_charm*alpha_charm))
    phi_init_bottom1 = (2.d0*sqrt((mass_bottom*alpha_bottom)**3)) * r * EXP(-r*(mass_bottom*alpha_bottom))
    phi_init_charm2 = (2.d0*sqrt((mass_charm*alpha_charm)**3)) * r * EXP(-r*(mass_charm*alpha_charm))
    phi_init_bottom2 = (2.d0*sqrt((mass_bottom*alpha_bottom)**3)) * r * EXP(-r*(mass_bottom*alpha_bottom))
    ! phi_init_mu = 0.d0
    ! phi_init_mu(1) = 1.d0
    ...
    DO i=1,n
        WRITE(1,'(2f15.8)') r(i),phi_init_charm1(i)
        WRITE(2,'(2f15.8)') r(i),phi_init_bottom1(i)
    END DO
    CLOSE(1)
    CLOSE(2)
...
!!!!!!!!!!!!         BEGINNING OF THE SCF PROCEDURE           !!!!!!!!!!!!!
    ...
    h_charm = -0.417d0/mass_charm * laplacian(r)
    h_bottom = -0.417d0/mass_bottom * laplacian(r)
    energy_old = 1.d5
    ...
    DO step = 1,max_step
        ALLOCATE(F_matrix_charm1(n,n),F_matrix_bottom1(n,n))
        ALLOCATE(F_matrix_charm2(n,n),F_matrix_bottom2(n,n))
        F_matrix_charm1 = h_charm -8.d0/9.d0*alpha_charm*Hartree(r,phi_init_charm2) &
            -4.d0/3.d0 * alpha_cb*Hartree(r,phi_init_bottom2)

        F_matrix_bottom2 = h_bottom -4.d0/9.d0*alpha_bottom*Hartree(r,phi_init_bottom1) &
            -2.d0/3.d0 * (alpha_cb*Hartree(r,phi_init_charm1))!+alpha_bottom*Hartree(r,phi_init_bottom2))

        F_matrix_charm2 = h_charm -4.d0/9.d0*alpha_charm*Hartree(r,phi_init_charm1) &
            -2.d0/3.d0 * (alpha_cb*Hartree(r,phi_init_bottom1))!+alpha_charm*Hartree(r,phi_init_charm2))

        F_matrix_bottom1 = h_bottom -8.d0/9.d0*alpha_bottom*Hartree(r,phi_init_bottom2) &
            -4.d0/3.d0 * alpha_cb*Hartree(r,phi_init_charm2)

        CALL diagonalize(F_matrix_charm1,eps_charm1,phi_charm1)
        CALL diagonalize(F_matrix_charm2,eps_charm2,phi_charm2)
        CALL diagonalize(F_matrix_bottom1,eps_bottom1,phi_bottom1)
        CALL diagonalize(F_matrix_bottom2,eps_bottom2,phi_bottom2)

        phi_init_charm1 = phi_charm1/sqrt(integrate(r,phi_charm1**2))
        phi_init_charm2 = phi_charm2/sqrt(integrate(r,phi_charm2**2))
        phi_init_bottom1 = phi_bottom1/sqrt(integrate(r,phi_bottom1**2))
        phi_init_bottom2 = phi_bottom2/sqrt(integrate(r,phi_bottom2**2))


        write(*,*) eps_bottom1,eps_charm1,eps_bottom2,eps_charm2
        
        ! phi_init_charm = phi_charm/sqrt(integrate(r,phi_charm**2))

        energy_new = 0.5d0*(eps_charm1+eps_bottom1 + 2.d0*eps_charm2 + 2.d0*eps_bottom2 +&
                integrate(r,phi_init_charm1*MATMUL(h_charm,phi_init_charm1)) + &
                integrate(r,phi_init_bottom1*MATMUL(h_bottom,phi_init_bottom1)) + &
                2.d0*integrate(r,phi_init_charm2*MATMUL(h_charm,phi_init_charm2)) + &
                2.d0*integrate(r,phi_init_bottom2*MATMUL(h_bottom,phi_init_bottom2)))
        ...
        deallocate(F_matrix_charm1,F_matrix_bottom1,F_matrix_charm2,F_matrix_bottom2)
    END DO
    ...
    WRITE(13,*)'Ground State Mass (MeV):', energy_new+3.d0*mass_charm+3.d0*mass_bottom
    WRITE(*,*)
    WRITE(*,*)'Ground State Mass (MeV):', energy_new+3.d0*mass_charm+3.d0*mass_bottom
