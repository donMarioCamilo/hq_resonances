PROGRAM main
    ...
    ! Parameters
    INTEGER, PARAMETER  :: n_orbitals = 2
    INTEGER, PARAMETER  :: n_atoms = 2
    INTEGER, PARAMETER  :: Z(n_atoms) = [1,1]
    INTEGER, PARAMETER  :: n_occupation = 1
    INTEGER, PARAMETER  :: max_it = 30
    DOUBLE PRECISION, PARAMETER :: scf_threshold = 1E-5
    
    DOUBLE PRECISION    :: alpha(n_orbitals,3)
    DOUBLE PRECISION    :: coeff(n_orbitals,3)
    DOUBLE PRECISION    :: coord(n_orbitals,3)
    INTEGER             :: l(n_orbitals,3)
    DOUBLE PRECISION    :: nuc_nuc_repulsion_E
    
    DOUBLE PRECISION    :: S_matrix(n_orbitals,n_orbitals), K_matrix(n_orbitals,n_orbitals),&
                             nuc_atraction(n_orbitals,n_orbitals), Two_e(n_orbitals,n_orbitals,n_orbitals,n_orbitals)
    DOUBLE PRECISION    :: X_trans(n_orbitals,n_orbitals)
    DOUBLE PRECISION    :: electronic_energy, energy_new, energy_old, energy_min, bond_length
    
    DOUBLE PRECISION    :: step, distance_i, bond_distances(500)
    INTEGER             :: distance_iterator
    ...
    bond_distances(1) = 0.4d0
    bond_distances(SIZE(bond_distances)) = 6.0d0
    step = (bond_distances(SIZE(bond_distances))-bond_distances(1))/SIZE(bond_distances)
    DO distance_iterator = 2,SIZE(bond_distances)-1
        bond_distances(distance_iterator) = bond_distances(distance_iterator-1) + step
    END DO
    
    energy_min = 1000.0d0
    energy_new = energy_min
    OPEN(unit = 12, file = 'energies.dat', status = 'replace', action = 'write')
    
    
    DO distance_iterator = 1, SIZE(bond_distances)
        energy_old = energy_new
        distance_i = bond_distances(distance_iterator)
        coord = 0.0d0
        coord(1,:) = [0.0d0, 0.0d0,0.0d0]
        coord(2,:) = [0.0d0, 0.0d0, distance_i]
        ...
        electronic_energy = scf_loop(n_orbitals, n_occupation, X_trans, K_matrix,&
                             nuc_atraction, Two_e, scf_threshold, max_it)
        energy_new = electronic_energy + nuc_nuc_repulsion_E
        
        WRITE(12,'(2f8.4)') distance_i, energy_new

        IF (energy_new < energy_min) THEN
            energy_min = energy_new
            bond_length = distance_i
        END IF
        
    END DO
    WRITE(*,'(A,f8.4,A,f8.4)') 'Bond length (a.u.):', bond_length, '    Energy (Hartree):', energy_min
    CLOSE(12)
    ...
    FUNCTION give_4matrix(n_orbitals,name)
            ...
            INTEGER, INTENT(IN)             :: n_orbitals
            CHARACTER(len=*), INTENT(IN)    :: name
            DOUBLE PRECISION                :: give_4matrix(n_orbitals,n_orbitals,n_orbitals,n_orbitals)
            
            INTEGER                         :: i, j, k, l, iread, jread, kread, lread
            DOUBLE PRECISION                :: element
            
            OPEN(unit = 11, file = name, status = 'OLD', action = 'READ')
            
            DO iread = 1,n_orbitals
                DO jread = 1,n_orbitals
                    DO kread = 1,n_orbitals
                        DO lread = 1,n_orbitals
                            READ(11, '(i3, i3, i3, i3, f8.4)') i,j,k,l, element
    ...
    FUNCTION scf_loop(n_orbitals, n_occupation, X_trans, K_matrix, nuc_atraction, Two_e, threshold, max_it) RESULT(energy_new)
            IMPLICIT NONE
            ...
            DOUBLE PRECISION, INTENT(IN)        ::  X_trans(n_orbitals,n_orbitals), K_matrix(n_orbitals,n_orbitals),&
