...
!In this demo the bbcc tetraquark ground state will be computed
PROGRAM HF_ccbb
    IMPLICIT none
...
!!!!!!!!!!!!         BEGINNING OF THE SCF PROCEDURE           !!!!!!!!!!!!!
    ...
    h_charm = -0.5d0/mass_charm * laplacian(r)
    h_bottom = -0.5d0/mass_bottom * laplacian(r)
    energy_old = 1000.d0
    ...
    DO step = 1,max_step
        ...
        F_matrix_bottom = h_bottom - alpha*Hartree(r,phi_init_charm) &
        -1.d0/3.d0*alpha_bottom*Hartree(r,phi_init_bottom)
        CALL diagonalize(F_matrix_bottom,eps_bottom,phi_bottom)
        phi_init_bottom = phi_bottom/sqrt(integrate(r,phi_bottom**2))


        F_matrix_charm = h_charm - alpha*Hartree(r,phi_init_bottom) &
        -1.d0/3.d0*alpha_charm*Hartree(r,phi_init_charm)
        ...
        energy_new = eps_charm+eps_bottom + &
    ...
    WRITE(13,*)'Ground State Mass (MeV):', energy_new+2.d0*mass_charm+2.d0*mass_bottom
    WRITE(*,*)
    WRITE(*,*)'Ground State Mass (MeV):', energy_new+2.d0*mass_charm+2.d0*mass_bottom
